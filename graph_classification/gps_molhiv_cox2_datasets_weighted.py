...
class BinaryFocalLoss(nn.Module):
    ...
    - `alpha` (float): Weighting factor for the positive class (e.g., 0.25).
    ...
    def __init__(self, alpha=0.25, gamma=2.0, reduction='mean'):
        ...
        # Use reduction='none' to apply focal term and alpha before reducing
        self.bce_loss = nn.BCEWithLogitsLoss(reduction='none')

    def forward(self, logits, targets):
        # logits: [N, ...], raw model output
        # targets: [N, ...], float labels
        ...
        if self.alpha is not None:
            # Alpha term: alpha if y=1, (1-alpha) if y=0
...
if __name__=="__main__":
    ...
    # --- ADDED: Class Weight Calculation Variables ---
    pos_weight_tensor = None
    actual_rwse_dim = 0 # Initialize
    ...
    if args.dataset in ["ogbg-molhiv"]:
        ...
        # --- ADDED: Calculate weights for OGB ---
        if args.use_class_weight:
            y_train = dataset.y[train_idx].squeeze()
            pos_samples = (y_train == 1).sum().item()
            neg_samples = (y_train == 0).sum().item()
        ...
        if args.use_class_weight:
            # Assuming binary TU dataset as per script's model/train logic
    ...
    if in_dim == 0:
        in_dim = 1
        # This fix is needed for TU datasets without node features
    ...
    for run in range(1,args.runs+1):
        ...
        model = GPS(in_dim=in_dim, channels=channels, pe_dim=actual_rwse_dim,
                    num_layers=args.num_layers, conv_type=args.conv_type, edge_dim=edge_dim, pool=args.pool).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=1e-5)
        
        # --- MODIFIED: Criterion Selection ---
        # This script is binary-only (model output dim = 1)
        ...
        scheduler = LambdaLR(optimizer, lr_lambda=lambda e: min(1.0, (e+1)/args.warmup_epochs)) # Corrected warmup
    ...
    for run in range(1, args.runs + 1):
        ...
        if os.path.exists(run_metrics_path): # Check if file exists
