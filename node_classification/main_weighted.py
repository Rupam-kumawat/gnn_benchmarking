...
# =====================================================================================
#  Section 1.5: ADDED LOSS FUNCTIONS
# =====================================================================================
...
class FocalLoss(nn.Module):
    ...
    - `alpha` (torch.Tensor): A tensor of weights for each class [C].
    - `gamma` (float): Focusing parameter (e.g., 2.0).
    """
    ...
    def forward(self, log_probs, targets):
        # log_probs: [N, C] (log_softmax output)
        # targets: [N] (class indices)
        
        # Get log_prob of the correct class
        # log_pt = NLLLoss(log_probs, targets, reduction='none')
...
def generate_param_string(args):
    ...
    # --- ADDED: Loss Hyperparameters ---
    if args.use_class_weight: params.append('class_weight')
    if args.use_focal_loss: params.append(f'focal_g-{args.focal_gamma}')
    if args.use_focal_loss and args.dataset in ('questions'): params.append(f'focal_a-{args.focal_alpha}')
...
def train_and_evaluate(args):
    ...
    print('args.use_class_weight', args.use_class_weight)
    
    if args.use_class_weight:
        print("--- Calculating Class Weights ---")
        if args.dataset in ('questions'): # Binary/Multilabel
            # For BCE, pos_weight = neg_count / pos_count
            y = dataset.label.squeeze().float() # [N] or [N, C]
            if y.dim() == 1:
                pos_samples = y.sum()
                total_samples = len(y)
            ...
            print(f"Binary problem. pos_weight = {pos_weight}")
        
        else: # Multiclass
            y = dataset.label.squeeze()
            class_counts = torch.bincount(y)
            # Use inverse frequency: total / (n_classes * count)
            class_weights = y.shape[0] / (c * class_counts.float())
            # Handle potential div by zero if a class is missing
            class_weights[torch.isinf(class_weights)] = 0.0 
            class_weights = class_weights.to(device)
            print(f"Multiclass problem. Weights: {class_weights}")
    ...
    for run in range(args.runs):
        ...
        # summary_dir = f'{args.base_results_dir}/{args.dataset}/{args.gnn}/{param_string}/summary'
        out_dir = f'{args.base_results_dir}/{args.dataset}/{args.gnn}/{param_string}/run_{run}'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--use_class_weight', action='store_true',
                        help='Use inverse class frequency to weight the loss function (pos_weight for BCE, weight for NLL).')
    ...
    parser.add_argument('--focal_alpha', type=float, default=0.25,
                        help='Alpha balancing parameter for Binary Focal Loss (weight for positive class). '
                             'For multiclass, class weights are used if --use_class_weight is set.')
