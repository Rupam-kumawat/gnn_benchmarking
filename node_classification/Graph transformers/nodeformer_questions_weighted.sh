#!/bin/bash
# ========================================================================
# NodeFormer Questions Experiment Sweep
# GPU: 0 (Change as needed)
# Dataset: questions
  ...
  for LAYER in "${LAYERS[@]}"; do
    for LOSS in "${LOSSES[@]}"; do
      for METRIC in "${METRICS[@]}"; do
        ...
        echo ">>> NODEFORMER RUN: dim=$HIDDEN, layers=$LAYER, loss=$LOSS, metric=$METRIC"
        
        python main_nodeformer_weighted.py \
          --dataset $DATASET \
          --gnn $MODEL \
          --local_layers $LAYER \
...
echo "✅ NodeFormer sweep completed."