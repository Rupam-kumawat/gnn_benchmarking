...
# Dedicated training script for NodeFormer: Focal Loss + Validation Logging + Flexible Dir Logic.
...
def plot_visualizations(tsne_embeds, tsne_labels, logits, labels, out_dir):
    ...
    plt.legend(); plt.title('t-SNE of Node Embeddings'); plt.savefig(os.path.join(out_dir, 'tsne_plot.png')); plt.close()
...
def train_and_evaluate(args):
    ...
    model = NodeFormerAdapted(
        ...
        local_layers=args.local_layers, dropout=args.dropout,
        heads=args.num_heads, res=args.res, ln=args.ln, jk=args.jk
    ).to(device)
    
    criterion = FocalLoss(gamma=args.focal_gamma) if args.use_focal else \
                (nn.NLLLoss() if args.dataset not in ('questions') else nn.BCEWithLogitsLoss())

    eval_func = eval_rocauc if args.metric == 'rocauc' else eval_acc
    logger = Logger(args.runs, args)
    
    all_runs = []
    exp_name = f"l_{args.local_layers}-h_{args.hidden_channels}-focal_{args.use_focal}"
    ...
    for run in range(args.runs):
        ...
        for epoch in range(args.epochs):
            ...
            if args.dataset in ('questions'):
                ...
                loss = criterion(out[train_idx], dataset.label.squeeze(1)[train_idx]) if args.use_focal else \
                       criterion(F.log_softmax(out, dim=1)[train_idx], dataset.label.squeeze(1)[train_idx])
            ...
            if epoch % args.display_step == 0:
                print(f'Run: {run+1:02d}, Epoch: {epoch:02d}, Loss: {loss:.4f}, Valid: {100*result[1]:.2f}%')
        ...
        with torch.no_grad():
            ...
            probs = torch.exp(out_log[test_idx])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NodeFormer Training')
