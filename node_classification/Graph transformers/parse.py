...
from network.gps_model import GPSModel
from polynormer_adapted import PolynormerAdapted
# from tunedGNN.medium_graph.GTs_baselines.sgformer_adapted import SGFormer, GCN
from sgformer_adapted import SGFormerAdapted, GCN
# --- CHANGE 1: Import the correct class from the correct file ---
...
def parse_method(args, n, c, d, device):
    if args.model == "GPS":
        ...
        model = PolynormerAdapted(d, args.hidden_channels, c, local_layers=args.local_layers, global_layers=args.global_layers,
            in_dropout=args.in_dropout, dropout=args.dropout, global_dropout=args.global_dropout,
            heads=args.num_heads, beta=args.beta, pre_ln=args.pre_ln).to(device)
            
    # --- CHANGE 2: Instantiate the correct class with the correct arguments ---
    elif args.model == 'nodeformer':
        model = NodeFormerAdapted(in_channels=d,
                                  hidden_channels=args.hidden_channels,
                                  out_channels=c,
                                  local_layers=args.local_layers, # Use local_layers
                                  dropout=args.dropout,
                                  heads=args.num_heads,
                                  ln=args.ln, # Map to LayerNorm
                                  res=args.res, # Map to residual connections
                                  jk=args.jk # Map to jumping knowledge
                                  ).to(device)
        ...
        gnn = GCN(in_channels=d,
                  ...
                  num_layers=args.layers,
                  dropout=args.dropout)
        model = SGFormerAdapted(d, args.hidden_channels, c, num_layers=args.layers, alpha=args.alpha, dropout=args.dropout, 
                         num_heads=args.num_heads, use_bn=args.use_bn, use_residual=args.use_residual, 
                         use_graph=args.use_graph, use_weight=args.use_weight, use_act=args.use_act, 
                         graph_weight=args.graph_weight, gnn=gnn, aggregate=args.aggregate, jk=args.jk).to(device)
    elif args.model == 'exphormer':
...
# The parser_add_main_args function does not need any changes.
def parser_add_main_args(parser):
    # dataset and evaluation
    parser.add_argument('--dataset', type=str, default='roman-empire')
    parser.add_argument('--data_dir', type=str, default='../data/')
    ...
    parser.add_argument('--metric', type=str, default='acc', choices=['acc','prauc', 'rocauc', 'qwk'],
                        help='evaluation metric')
    # ---- NOTE: The --model argument is now used to select 'nodeformer' ----
    parser.add_argument('--model', type=str, default='MPNN') 
    # GNN
    parser.add_argument('--gnn', type=str, default='gcn')
    parser.add_argument('--hidden_channels', type=int, default=256)
