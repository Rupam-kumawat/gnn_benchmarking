...
# Save this as nodeformer_adapted.py
...
def kernelized_softmax(query, key, value, kernel_transformation, projection_matrix=None, edge_index=None, tau=0.25, return_weight=True):
    ...
    if return_weight and edge_index is not None:
        start, end = edge_index
...
class NodeFormerConv(nn.Module):
    # ... (code is identical to the previous version)
    def __init__(self, in_channels, out_channels, num_heads, nb_random_features=10, rb_order=0, rb_trans='sigmoid', use_edge_loss=True):
        super(NodeFormerConv, self).__init__()
        self.Wk = nn.Linear(in_channels, out_channels * num_heads)
        self.Wq = nn.Linear(in_channels, out_channels * num_heads)
        self.Wv = nn.Linear(in_channels, out_channels * num_heads)
    ...
    def forward(self, z, adjs, tau):
        ...
        z_next, weight = kernelized_softmax(query, key, value, self.kernel_transformation, projection_matrix, adjs[0], tau, self.use_edge_loss)
        for i in range(self.rb_order):
            z_next += add_conv_relational_bias(value, adjs[i], self.b[i], self.rb_trans)
        z_next = self.Wo(z_next.flatten(-2, -1))
        if self.use_edge_loss and weight is not None:
            row, col = adjs[0]
            d_in = degree(col, query.shape[1]).float()
            d_norm = 1. / d_in[col]
            d_norm_ = d_norm.reshape(1, -1, 1).repeat(1, 1, weight.shape[-1])
...
# ===============================================================================================
# INTERNAL NODEFORMER CLASS (NOW REFACTORED)
# ===============================================================================================

class _NodeFormer(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2, num_heads=4, dropout=0.0,
                 use_bn=True, use_residual=True, use_act=False, use_jk=True, rb_order=2, use_edge_loss=False):
        super(_NodeFormer, self).__init__()
        self.convs = nn.ModuleList()
        self.fcs = nn.ModuleList()
        self.fcs.append(nn.Linear(in_channels, hidden_channels))
        self.bns = nn.ModuleList()
        ...
        for _ in range(num_layers):
            self.convs.append(NodeFormerConv(hidden_channels, hidden_channels, num_heads=num_heads,
                                            rb_order=rb_order, use_edge_loss=use_edge_loss))
            self.bns.append(nn.LayerNorm(hidden_channels))
        if use_jk:
            self.fcs.append(nn.Linear(hidden_channels * num_layers + hidden_channels, out_channels))
        else:
    ...
    # ### NEW METHOD ###
    # This method now contains the core logic for generating node embeddings.
    def get_embeddings(self, x, edge_index, tau=1.0):
        n = x.shape[0]
        adjs = []
        adj, _ = remove_self_loops(edge_index)
        adj, _ = add_self_loops(adj, num_nodes=n)
        ...
        x_unsqueezed = x.unsqueeze(0) # NodeFormer expects a batch dimension
        layer_ = []
        link_loss_ = []

        z = self.fcs[0](x_unsqueezed)
        if self.use_bn:
            z = self.bns[0](z)
        z = self.activation(z)
        z = F.dropout(z, p=self.dropout, training=self.training)
        ...
        for i, conv in enumerate(self.convs):
            z, link_loss = conv(z, adjs, tau)
            if self.use_edge_loss and link_loss is not None:
                link_loss_.append(link_loss)
            if self.use_residual:
                z += layer_[i]
            if self.use_bn:
        ...
        if self.use_jk:
            final_z = torch.cat(layer_, dim=-1)
        else:
            final_z = z
        
        # Squeeze the batch dimension before returning embeddings
        final_z_squeezed = final_z.squeeze(0)

        # Also return link_loss_ so the forward pass can use it
        return final_z_squeezed, link_loss_

    # ### UPDATED METHOD ###
    # The forward pass is now simpler, using get_embeddings.
    def forward(self, x, edge_index, tau=1.0):
        # 1. Get the final node embeddings (and any potential loss)
        z, link_loss_ = self.get_embeddings(x, edge_index, tau)

        # 2. Apply the final prediction head
        # Note: The unsqueeze/squeeze is handled inside get_embeddings and here.
        x_out = self.fcs[-1](z)
...
# ===============================================================================================
# THE NEW WRAPPER CLASS (NOW WITH get_embeddings)
# ===============================================================================================

class NodeFormerAdapted(torch.nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels,
                 local_layers=2, dropout=0.5, heads=4, jk=False,
                 pre_ln=False, pre_linear=False, res=True, ln=True, bn=False, gnn=None):
        
        super(NodeFormerAdapted, self).__init__()
        self.nodeformer = _NodeFormer(
    ...
    def reset_parameters(self):
        self.nodeformer.reset_parameters()

    # ### NEWLY ADDED METHOD ###
    # This method calls the internal model's get_embeddings and returns only the features,
    # matching the MPNNs class interface perfectly.
    def get_embeddings(self, x, edge_index):
        embeddings, _ = self.nodeformer.get_embeddings(x, edge_index)
        return embeddings

    # ### UPDATED METHOD ###
    # This method calls the internal model's forward pass.
    def forward(self, x, edge_index):
        output = self.nodeformer(x, edge_index)
